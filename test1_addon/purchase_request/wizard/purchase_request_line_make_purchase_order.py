...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    is_create_from_rfq = fields.Boolean(
        string="Create PO", readonly=True
    )
    ...
    def default_get(self, fields):
        ...
        request_id = self.env['purchase.request'].browse(active_id)
        min_rfq_count = int(self.env['ir.config_parameter'].sudo().get_param(
            'purchase_requests.min_rfq_count'))
        print('..request_id.is_create_po',request_id.is_create_po)
        res["is_create_from_rfq"] = False if request_id.is_create_po else True
        request_line_ids = []
    ...
    @api.model
    def _prepare_purchase_order(self, company, origin):
    ...
    def make_purchase_order(self):
        ...
        if self.rfq_id:
            ...
            purchase = self.rfq_id.button_confirm()
            purchase.request_id = request_id
        else:
