...
class QuarryGeneralworkRequestController(http.Controller):

    @http.route(['/quarry_work/create'], type='json', auth='public', methods=['POST'], csrf=False)
    def hms_quarry_general_work_post(self, **post):
        ...
        if params:
            ...
            if 'work_head_id' in params:
                work_head_id = int(params.get('work_head_id'))
                log_vals.update({'work_head_id': work_head_id or False})

            if 'sub_contractor' in params:
                sub_contractor = int(params.get('sub_contractor'))
                log_vals.update({'sub_contractor': sub_contractor or False})

            if 'particulars' in params:
                particulars = params.get('particulars')
                log_vals.update({'particulars': particulars or False})
            ...
            if 'space' in params:
                ...
                log_vals.update({'space': space or False})
            ...
            if params['username'] and params['password']:
                ...
                try:
                    ...
                    response = models.execute_kw(request.env.cr.dbname, user_id.id, str(params['password']),
                                                 'quarry_work.api.create',
                                                 'action_create_quarry_general_work', [log_vals])

                    quarry_dict = json.loads(response)
                    return json.dumps({
                        'status': quarry_dict['status'],
                        'status_code': quarry_dict['status_code'],
                        'quarry_general_work_id': quarry_dict['quarry_general_work_id'],
                    })
