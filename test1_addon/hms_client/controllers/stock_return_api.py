...
class StockReturnController(http.Controller):

    @http.route(['/stock_return/create'], type='json', auth='public', methods=['POST'], csrf=False)
    def stock_return_create(self, **post):
        ...
        if params:
            ...
            if 'return_from_location' in params:
                return_from_location = int(params.get('return_from_location'))
                log_vals.update({'return_from_location': return_from_location or False})

            if 'return_to_location' in params:
                return_to_location = int(params.get('return_to_location'))
                log_vals.update({'return_to_location': return_to_location or False})
            ...
            if params['username'] and params['password']:
                ...
                try:
                    ...
                    response = models.execute_kw(request.env.cr.dbname, user_id.id, str(params['password']),
                                                 'stock_return.api.create',
                                                 'action_create_stock_return', [log_vals])
                    stock_return_dict = json.loads(response)  # converted

                    status = 'SUCCESS'
                    status_code = 200
                    return json.dumps({
                        'status': stock_return_dict['status'],
                        'status_code': stock_return_dict['status_code'],
                        'stock_return_id': stock_return_dict['stock_return_id'],
                        'receipt_id': stock_return_dict['stock_receipt_id'],
                    })
